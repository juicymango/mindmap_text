...
# Task 49: Generate Prompt Feature Implementation Plan

## Overview

Task 49 requires adding a "Generate Prompt" feature that creates an LLM prompt based on the selected node's context, including the selected path and sibling nodes. The generated prompt should be copied to clipboard and guide the LLM to generate hierarchical JSON mind map content.

## Feature Requirements

### Core Functionality
1. **Generate Prompt Button**: Add to toolbar for selected node operations
2. **Query Input Dialog**: Modal dialog for user to enter their question/query
3. **Context Generation**: Include selected node, path, and sibling context
4. **Prompt Generation**: Create comprehensive LLM prompt with JSON output instructions
5. **Clipboard Integration**: Copy generated prompt to clipboard automatically

### Technical Requirements
- Generate prompt only (no actual LLM API calls)
- Include hierarchical context from selected path
- Ask LLM to generate results in JSON mind map format
- Use hierarchical structure, don't push all content to root node
...
#### New Store Methods
```typescript
// Generate prompt context from selected node
generatePromptContext: (path: number[]) => {
  // Returns context object with:
  // - selectedNode: MindNode
  // - selectedPath: number[]
  // - parentNodes: MindNode[]
  // - siblingNodes: MindNode[]
  // - childNodes: MindNode[]
  // - fullContext: string representation
}
```

#### Context Generation Logic
1. **Find Selected Node**: Use existing `findNode` function
2. **Collect Path Context**: Walk up the path to get parent context
3. **Get Siblings**: Find all children of each parent in the path
4. **Get Children**: Include direct children of selected node
5. **Format Context**: Create structured context object

### 2. UI Components

#### Toolbar Enhancement
```typescript
// Add to existing toolbar buttons
<button 
...
const PromptDialog: React.FC<PromptDialogProps> = ({
  ...
  // - Generated prompt display (after generation)
...
interface PromptContext {
  selectedNode: MindNode;
  selectedPath: number[];
  parentNodes: { node: MindNode; path: number[] }[];
  siblingNodes: { node: MindNode; path: number[] }[];
  childNodes: MindNode[];
  mindmapStructure: MindMap;
}

function buildPromptContext(path: number[]): PromptContext {
  // 1. Find selected node
  // 2. Build parent chain with paths
  // 3. Collect siblings at each level
  // 4. Get children of selected node
  // 5. Include full mindmap structure for reference
}
```

#### Prompt Template
```typescript
function generateLLMPrompt(query: string, context: PromptContext): string {
  return `
You are a helpful assistant that generates structured mind map content.

USER QUERY: ${query}

CURRENT CONTEXT:
Selected Node: ${context.selectedNode.text}
Selected Path: ${context.selectedPath.join(' > ')}

PARENT CONTEXT:
${context.parentNodes.map(p => `- ${p.node.text} (path: ${p.path.join(' > ')})`).join('\n')}

SIBLING NODES:
${context.siblingNodes.map(s => `- ${s.node.text}`).join('\n')}

CHILD NODES:
${context.childNodes.map(c => `- ${c.text}`).join('\n')}

INSTRUCTIONS:
1. Generate a response that addresses the user's query
2. Structure your response as a hierarchical mind map in JSON format
3. Use the following JSON structure:
   {
     "text": "Main response topic",
     "children": [
       {
         "text": "Subtopic 1",
         "children": [...]
...
4. Create a logical hierarchy - don't put all content at the root level
5. Keep each node focused on a single concept
6. Use clear, concise text for node labels
7. Generate 3-7 main branches with appropriate sub-branches

Please generate only the JSON response without additional explanation.
`;
}
```
...
### Phase 1: Store and Context Logic (2-3 hours)
1. **Implement `generatePromptContext` in store**
2. **Create context building utilities**
3. **Add TypeScript types for prompt context**
4. **Write unit tests for context generation**

### Phase 2: UI Components (3-4 hours)
1. **Add Generate Prompt button to Toolbar**
2. **Create PromptDialog component**
3. **Implement modal state management**
4. **Style dialog to match existing UI**
5. **Add responsive design considerations**

### Phase 3: Prompt Generation (2-3 hours)
1. **Implement prompt generation logic**
2. **Create prompt template system**
3. **Add clipboard copy functionality**
4. **Handle copy success/failure feedback**
...
2. **Add feature explanation to UI design doc**
3. **Update code structure documentation**
...
describe('generatePromptContext', () => {
  it('should generate correct context for root node selection', () => {
    // Test root node context generation
  });

  it('should include parent nodes in context', () => {
    // Test parent chain inclusion
  });

  it('should include sibling nodes at each level', () => {
    // Test sibling node collection
  });

  it('should include children of selected node', () => {
    // Test children inclusion
  });
});
```
...
describe('generateLLMPrompt', () => {
  ...
  it('should format context information correctly', () => {
...
describe('PromptDialog', () => {
  ...
  it('should copy generated prompt to clipboard', () => {
...
### Integration Tests
- **End-to-end workflow**: Button click → Dialog open → Query input → Prompt generation → Clipboard copy
- **Error handling**: Invalid contexts, clipboard failures, empty queries
...
- **Invalid Path**: Handle non-existent node paths gracefully
- **Empty Context**: Provide meaningful default context
- **Large Context**: Handle performance with very large mind maps

### UI Error Handling
- **Clipboard Failures**: Show user-friendly error messages
- **Empty Queries**: Validate input before generation
- **Modal State**: Ensure proper cleanup and state management

### Accessibility Considerations
- **Keyboard Navigation**: Full keyboard support for dialog
- **Screen Reader Support**: Proper ARIA labels and announcements
- **Focus Management**: Correct focus trapping in modal
- **High Contrast**: Ensure visibility in high contrast mode

## Success Criteria

### Functional Requirements
- ✅ Generate Prompt button appears in toolbar when node is selected
- ✅ Dialog opens with context information displayed
- ✅ User can input query and generate prompt
- ✅ Generated prompt includes selected node context
- ✅ Generated prompt includes path and sibling context
- ✅ Prompt includes JSON output instructions
- ✅ Generated prompt copied to clipboard automatically
- ✅ User feedback for successful/failed operations

### Quality Requirements
- ✅ All new tests passing (15-20 new test cases)
- ✅ TypeScript compilation successful
- ✅ No ESLint errors
...
- ✅ Context generation performs well with large mind maps
- ✅ UI remains responsive during operations
- ✅ Memory usage remains reasonable

## Timeline Estimate

- **Phase 1 (Store/Logic)**: 2-3 hours
- **Phase 2 (UI Components)**: 3-4 hours
- **Phase 3 (Prompt Generation)**: 2-3 hours
- **Phase 4 (Testing)**: 2-3 hours
- **Phase 5 (Documentation)**: 1-2 hours
...
### Dependencies
- Existing store structure and node finding utilities
- Existing UI components and styling system
- Existing clipboard utilities and error handling patterns
- React state management and hooks

### Assumptions
- User has basic understanding of LLM prompting
- Generated prompts will be used with external LLM services
- Clipboard API is available (with fallbacks for older browsers)
- Existing mind map structure and utilities are stable

## Risk Assessment

### Medium Risk Areas
1. **Context Generation Complexity**: Building meaningful context from hierarchical structure
   - **Mitigation**: Clear algorithm, comprehensive testing
2. **UI Integration**: Integrating new dialog with existing toolbar and state management
   - **Mitigation**: Follow existing patterns, incremental implementation
3. **Clipboard API Compatibility**: Browser variations in clipboard implementation
   - **Mitigation**: Use existing clipboard utilities with fallbacks

### Low Risk Areas
1. **Prompt Template**: Relatively straightforward string formatting
2. **Button Integration**: Following existing toolbar patterns
3. **Testing**: Well-defined test scenarios and coverage

This implementation plan provides a comprehensive approach to adding the Generate Prompt feature while maintaining code quality and user experience standards.